...
class BrailleMultilineSettingsPanel(gui.settingsDialogs.SettingsPanel):
	...
	def makeSettings(self, settingsSizer):
		...
		# Here rather than in the flow settings, because it is about what a cell of this display
		# can draw and applies to everything shown on it. A menu bar in File Explorer is full of
		# buttons and never goes near a flow.
		# Translators: label of a checkbox in settings. Roles and states are the short words NVDA
		# writes to say what a thing is, such as "btn" for a button.
		glyphLabel = _("Draw ro&les and states as shapes instead of words")
		self.glyphCtrl = sHelper.addItem(wx.CheckBox(self, label=glyphLabel))
		self.glyphCtrl.SetValue(bool(section["drawGlyphs"]))
	...
	def onSave(self):
		...
		section["drawGlyphs"] = self.glyphCtrl.IsChecked()
